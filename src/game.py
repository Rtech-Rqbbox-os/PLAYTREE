...
class Game:
    ...
    def _load_game(self):
        ...
        for sid, lvl in data.get("skill_levels", {}).items():
            if sid in self.skill_tree.skills:
                self.skill_tree.skills[sid]["level"] = min(lvl, self.skill_tree.skills[sid]["max"])
    ...
    def _handle_shop_click_legacy(self, pos):
        mx, my = pos
    ...
    def _draw_shop(self):
        ...
        coming_soon_font = pygame.font.Font(None, 60)
        coming_soon_text = coming_soon_font.render("COMING SOON", True, GOLD)
        self.screen.blit(coming_soon_text, (WIDTH // 2 - coming_soon_text.get_width() // 2, HEIGHT // 2 - 40))

        date_font = pygame.font.Font(None, 36)
        date_text = date_font.render("11/07/2026", True, (200, 200, 200))
        self.screen.blit(date_text, (WIDTH // 2 - date_text.get_width() // 2, HEIGHT // 2 + 20))

        if self.controller_connected:
            xbox_icons.draw_menu_prompt(self.screen, [("S", "S", "Close")], sy + sh - 25)
        else:
            hint = sf_sm.render("S to close", True, (80, 100, 80))
            self.screen.blit(hint, (sx + 20, sy + sh - 25))
    ...
    def _draw_locker(self):
        ...
        tf = pygame.font.Font(None, 44)
        ts = tf.render("LOCKER", True, GOLD)
        self.screen.blit(ts, (lx + 20, ly + 10))

        cf = pygame.font.Font(None, 60)
        cs = cf.render("COMING SOON", True, GOLD)
        self.screen.blit(cs, (WIDTH // 2 - cs.get_width() // 2, HEIGHT // 2 - 30))

        sf = pygame.font.Font(None, 28)
        ss = sf.render("Customize your character with hats, auras, titles, and more!", True, (160, 160, 160))
        self.screen.blit(ss, (WIDTH // 2 - ss.get_width() // 2, HEIGHT // 2 + 20))

        if self.controller_connected:
            xbox_icons.draw_menu_prompt(self.screen, [("BACK", "ESC", "Close")], ly + lh - 25)
        else:
            hint = pygame.font.Font(None, 18).render("Press ESC to close", True, (80, 100, 80))
